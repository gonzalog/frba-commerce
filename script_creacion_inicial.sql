...
IF 
(
(@ESTADO = 'Finalizada') AND 
(THE_DISCRETABOY.ES_VENTA_DIRECTA(@ID) = 0) AND 
(THE_DISCRETABOY.TIENE_OFERTAS(@ID) = 1) AND
(THE_DISCRETABOY.ESTA_FINALIZADA(@ID) = 0)
)
BEGIN
...
GO
--RETORNA LA CANTIDAD DE PUBLICACIONES POR FACTURAR
CREATE PROC THE_DISCRETABOY.get_cant_a_fact
(@USER NVARCHAR(20))
AS
BEGIN
...
FROM
THE_DISCRETABOY.Publicacion P
LEFT JOIN THE_DISCRETABOY.Renglon_factura R ON
R.publicacion = P.id
WHERE
P.usuario = @USER AND
R.nro_renglon IS NULL AND
(
THE_DISCRETABOY.ESTA_FINALIZADA(P.id) = 1 OR
